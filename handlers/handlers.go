...
func Friendlist(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT u.*, f.id as request_id , f.request, f.created_at as added_at FROM users u INNER JOIN friends f  ON  f.user_id = u.id OR f.friend_id = u.id  WHERE u.id != ? AND f.request = ?", user_id, request)
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
